...
type StoredMap = Omit<OfflineMapV2, 'downloadedAt'> & {
  downloadedAt?: string;
};

type StoredQueueItem = Omit<SyncQueueItemV2, 'savedAt'> & {
  savedAt: string;
};
...
function reviveMap(map: StoredMap): OfflineMapV2 {
  return {
    ...map,
    downloadedAt: map.downloadedAt ? new Date(map.downloadedAt) : undefined,
  };
}
...
function serializeMaps(maps: OfflineMapV2[]): StoredMap[] {
  return maps.map((map) => ({
    ...map,
    downloadedAt: map.downloadedAt?.toISOString(),
  }));
}

function serializeQueue(queue: SyncQueueItemV2[]): StoredQueueItem[] {
...
export function OfflineProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      try {
        ...
        if (!mounted) return;
        if (storedMaps) {
          const parsed = JSON.parse(storedMaps) as StoredMap[];
          setMaps(parsed.map(reviveMap));
        }
        if (storedQueue) {
          const parsed = JSON.parse(storedQueue) as StoredQueueItem[];
          setSyncQueue(parsed.map(reviveQueueItem));
        }
      } finally {
        if (mounted) {
          setIsHydrated(true);
  ...
  useEffect(() => {
    ...
    void AsyncStorage.setItem(STORAGE_KEYS.maps, JSON.stringify(serializeMaps(maps)));
  }, [maps, isHydrated]);
  ...
  useEffect(() => {
    ...
    void AsyncStorage.setItem(
      STORAGE_KEYS.queue,
      JSON.stringify(serializeQueue(syncQueue)),
    );
  ...
  const downloadMap = useCallback((id: string) => {
    ...
    setMaps((prev) =>
      prev.map((map) =>
        map.id === id
          ? { ...map, status: 'downloading' as const, progress: 0 }
          : map,
      ),
    );
  ...
  const deleteMap = useCallback((id: string) => {
    const target = mapsRef.current.find((map) => map.id === id);
    if (!target) return;

    Alert.alert(
      'Eliminar mapa',
      `¿Eliminar "${target.name}" del dispositivo?`,
      [
  ...
  const retrySyncItem = useCallback((id: string) => {
    const existingTimeout = syncTimeoutsRef.current[id];
    if (existingTimeout) {
  ...
  useEffect(() => {
    ...
    const pendingIds = syncQueueRef.current
